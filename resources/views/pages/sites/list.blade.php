@extends('layouts.default')

@section('title', 'Sites')

@section('content')
    <!-- BEGIN PAGE CONTAINER-->
    <div class="page-content">
        <div class="clearfix"></div>
        <div class="content">
            <ul class="breadcrumb">
                <li><p>YOUR ARE HERE:</p></li>
                <li>Sites</li>
                <li><a href="#" class="active">List</a> </li>
            </ul>
            <div class="page-title">
                <i class="material-icons">location_city</i><h3>Sites</h3>
            </div>
            <div class="row-fluid">
                <div class="span12">
                    <div class="grid simple ">
                        <div class="grid-title">
                            <h4> &nbsp; <span class="semi-bold"></span></h4>
                        </div>
                        <div class="grid-body ">
                            <div class="table-responsive">
                                <table class="table table-striped dataTable" id="_table" width="100%">
                                    <thead>
                                    <tr>
                                        <th style="width:10%">NAME</th>
                                        <th style="width:10%">PHONE NUMBER</th>
                                        <th style="width:10%">EMAIL</th>
                                        <th style="width:10%">ADDRESS</th>
                                        <th style="width:10%">COORDINATES</th>
                                        <th style="width:10%">WEBSITE</th>
                                        <th style="width:10%">CONTACT PERSON</th>
                                        <th style="width:10%">COMPANY</th>
                                        <th style="width:10%">LEARNERS</th>
                                        <th style="width:10%">DEVICES</th>
                                        <th style="width:10%">ACTIONS</th>
                                    </tr>
                                    </thead>
                                    <tbody>

                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- END PAGE CONTAINER-->
@endsection
@section('footer')
    @parent

    <script>
        $(document).ready(function() {
            var table =  $('#_table').DataTable({
                ajax: "/api/sites/all" ,
                language : {
                    sLengthMenu: "_MENU_",
                    search:         "",
                    searchPlaceholder: "Search ..."
                },
                dom: "<'row'<'col-sm-1'l><'col-sm-1 text-center'B><'col-sm-10'f>>" +
                "<'row'<'col-sm-12'tr>>" +
                "<'row'<'col-sm-5'i><'col-sm-7'p>>",
                pageLength:  25,
                buttons: [
                    {
                        extend: 'collection',
                        text: '&nbsp; <i class="fa fa-cloud-download"></i> &nbsp; Download',
                        buttons: [
                            {
                                extend: 'excel',
                                text: 'Excel',
                                title: '{{env('APP_NAME')}} - Sites',
                                exportOptions: {
                                    columns: [0,1,2,3,4,5,6,7,8]
                                }
                            },
                            {
                                extend: 'pdf',
                                text: 'PDF',
                                title: '{{env('APP_NAME')}}  - Sites',
                                orientation: 'landscape',
                                pageSize: 'LEGAL',
                                exportOptions: {
                                    columns: [0,1,2,3,4,5,6,7,8]
                                }
                            }
                        ]
                    }
                ],
                columns: [
                    {   //NAME
                        data: 'name',
                        defaultContent: ''
                    },
                    {   //PHONE NUMBER
                        data: 'phone_number',
                        defaultContent: ''
                    },
                    {   //EMAIL
                        data: 'email',
                        defaultContent: ''
                    },
                    {   //ADDRESS
                        data: 'address.id',
                        defaultContent: 'N/a'
                    },
                    {   //COORDINATES
                        data: null,
                        defaultContent: '',
                        'render' : function ( data, type, row, meta ) {
                            return '<a href = "" class="btn btn-white btn-block btn-small" ><i class="fa fa-map-marker"></i> &nbsp; MAP </a>';
                        }
                    },
                    {   //WEBSITE
                        data: 'website',
                        defaultContent: 'N/a'
                    },
                    {   //CONTACT PERSON
                        data: 'contact_person',
                        defaultContent: ''
                    },
                    {   //COMPANY
                        data: 'company.name',
                        defaultContent: ''
                    },
                    {   //LEARNERS
                        data: 'learners',
                        defaultContent: ''
                    },
                    {   //DEVICES
                        data: 'device_count',
                        defaultContent: ''
                    },
                    {
                        //ACTIONS
                        data: null,
                        defaultContent: '',
                        'render' : function ( data, type, row, meta ) {
                            return '<a href = "/sites/edit/'+ data.id +'" class="btn btn-info btn-cons btn-block btn-small" ><i class="fa fa-paste"></i> &nbsp; EDIT </a>';
                        }
                    }
                ],
                filterDropDown: {
                    bootstrap: true,
                    label: "Filters | ",
                    columns: [
                        {idx: 7}

                    ]
                }
            });

            $('div.dataTables_length select').select2({minimumResultsForSearch: -1});
            $('#_table_filterSelect7').select2({
                minimumResultsForSearch: -1,
                width: '400px'
            });
        });
    </script>
@endsection